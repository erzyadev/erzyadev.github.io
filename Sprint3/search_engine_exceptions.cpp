...
vector<string> SplitIntoWords(const string &text)
{
    ...
    {
        if (IsSpecial(c))
            throw invalid_argument("Special symbol encountered");
...
template <typename StringContainer>
const StringContainer &VerifyStrings(const StringContainer &strings)
{
    for (auto &word : strings)
    {
        if (!IsValidWord(word))
            throw invalid_argument("Special character encountered");
    }
    return strings;
}
...
public:
    ...
    explicit SearchServer(const StringContainer &stop_words)
        : stop_words_(MakeUniqueNonEmptyStrings(VerifyStrings(stop_words))) //Verify raw strings s
    {
    }
    ...
    tuple<vector<string>, DocumentStatus> MatchDocument(const string &raw_query, int document_id) const
    {
        ...
        return {move(matched_words), documents_.at(document_id).status};
    }
