...
namespace json_reader
{
    ...
    namespace
    {
        ...
        struct StopRequest : public StatRequest
        {
            std::string name;
            StopRequest(int id, std::string name) : StatRequest{id}, name(move(name)) {}
            json::Node GetProcessed(const request_handler::RequestHandler &handler) const override
            {
                return MakeStopStatsNode(handler.GetStopStats(name).stop_data, id);
            }
        };

        struct BusRequest : public StatRequest
        {
            std::string name;
            BusRequest(int id, std::string name) : StatRequest{id}, name(move(name)) {}

            json::Node GetProcessed(const request_handler::RequestHandler &handler) const override
            {
                return MakeBusStatsNode(handler.GetBusStats(name).bus_stats, id);
            }
        };

        struct MapRequest : public StatRequest
        {
        ...
        unique_ptr<json_reader::StatRequest> ReadStatRequestFromJsonMap(const json::Dict &request_map)
        {
            ...
            auto &request_type = request_map.at("type").AsString();
            if (request_type == "Map"sv)
            {

                return make_unique<json_reader::MapRequest>(id);
            }
            else if (request_type == "Stop"sv)
            {
                return make_unique<json_reader::StopRequest>(id, request_map.at("name").AsString());
            }
            else if (request_type == "Bus"sv)
            {
                return make_unique<json_reader::BusRequest>(id, request_map.at("name").AsString());
            }
            else if (request_type == "Route"sv)
            {
                return make_unique<json_reader::RouteRequest>(id, request_map.at("from").AsString(), request_map.at("to").AsString());
            }
            else
                throw std::invalid_argument("Incorrect statistic request");
        }
    }
